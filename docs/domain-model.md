...
- It allows calendar behavior similar to common recurring-event editors.

## 4. Supporting Enums

### 4.1 Task Status

`task_status` captures the current user-visible lifecycle of the task definition.
...
- Should single-occurrence edits allow both time and instruction overrides in MVP, or time only?
- Should archived tasks remain queryable in the same task detail endpoint or move to a separate history surface?
