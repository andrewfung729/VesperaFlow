...
and Pi executor preflight, runtime classification, and artifacts are implemented.
...
| Worker | Partial | TaskRunWorkflow, activities, debug printer, Claude Agent SDK executor, Codex CLI executor, OpenCode CLI executor, Pi CLI JSON executor, recurring materialization, and import-hygiene/replay coverage exist; authenticated live executor and recurring smokes are opt-in local verification paths. |
...
- Claude Agent SDK, Codex CLI, OpenCode CLI, and Pi CLI JSON executors, runtime classification, and
