...
profile-primary with no install-level fallback; Claude Code, Codex, OpenCode, and Kimi
...
| Worker | Partial | TaskRunWorkflow, activities, debug printer, Claude Agent SDK executor, Codex CLI executor, OpenCode CLI executor, Kimi CLI text executor, recurring materialization, and import-hygiene/replay coverage exist; authenticated live executor and recurring smokes are opt-in local verification paths. |
...
- Claude Agent SDK, Codex CLI, OpenCode CLI, and Kimi CLI text executors, runtime classification, and
