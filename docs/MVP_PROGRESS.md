...
_Last updated: 2026-05-19. Update this whenever product scope or implementation
status changes._
...
profile-primary with no install-level fallback; Claude Code, Codex, OpenCode,
Kimi Code, and Pi executor preflight, runtime classification, and artifacts are implemented.
...
|---|---|---|---|
| Create one-time deferred task | Partial | API route and repository create a task, single-run schedule, planned run, target working directory, and Temporal Schedule; web composer creates one-time tasks and gates Claude Code, Codex, OpenCode, Kimi Code, and Pi task creation through executor preflight; an opt-in full-stack `debug_printer` smoke test covers local Postgres, Temporal, API, Worker, and persisted run state. Run-now endpoint and UI button allow triggering a planned one-time task immediately. | Need a recorded live success path for the selected production executor. Full-stack smoke stays opt-in and local-only; it will not become a required CI gate for MVP. |
...
| Review planned and completed work | Partial | Task detail returns task, schedule, and latest run preview; run list/history payloads now use preview fields (`outcome_preview`, `outcome_truncated`, `outcome_source`) while full outcome content and the effective instruction snapshot are served by `GET /api/v1/runs/{run_id}`, `GET /api/v1/tasks/{task_id}/runs/{run_id}`, and `GET /api/v1/tasks/{task_id}/runs/{run_id}/reader`; one-time board groups tasks by status; `/api/v1/views/history` and the web History route show completed/failed runs across tasks with status and execution-mode filters; `/api/v1/views/recurring-todo` and the web recurring todo route show active/paused recurring commitments with latest-run context; `/api/v1/views/calendar` and the web Calendar route show one-time and recurring planned work across day/week/month views. Recurring materialized runs use the same run/history model. | Broaden web behavior coverage beyond smoke-level. |
...
| History view for run review | Done | Store history query, `/api/v1/views/history`, and `HistoryView.vue` list completed/failed runs in reverse finished order with preview payloads, filter by outcome and execution mode, and link all items to the run detail workbench (`/tasks/{task_id}/runs/{run_id}`), which can open the focused reader (`/tasks/{task_id}/runs/{run_id}/read`). Materialized recurring runs use the same read model. Repository, API, web unit, and Playwright smoke coverage exist. | Future hardening can add richer saved filters and bulk review actions. |
| Executor integration | Done | Worker workflow, activities, executor router, debug printer, Claude Agent SDK invocation, Codex CLI JSONL invocation, OpenCode CLI JSON event invocation, Kimi CLI text invocation, Pi CLI JSON invocation, API workspace preflight endpoints for production executors, profile-primary selection with no install-level fallback, and composer preflight UX exist; Claude, Codex, OpenCode, Kimi, and Pi output artifacts and auth, misconfiguration, workspace, timeout/cancellation, and execution failure classifications are covered with mocked Worker/API/web tests. Authenticated live executor smokes are documented as opt-in local verification paths. | Full-stack smoke stays opt-in and local-only; it will not become a required CI gate for MVP. |
...
| Worker | Partial | TaskRunWorkflow, activities, debug printer, Claude Agent SDK executor, Codex CLI executor, OpenCode CLI executor, Kimi CLI text executor, Pi CLI JSON executor, recurring materialization, and import-hygiene/replay coverage exist; authenticated live executor and recurring smokes are opt-in local verification paths. |
...
- Claude Agent SDK, Codex CLI, OpenCode CLI, Kimi CLI text, and Pi CLI JSON executors, runtime classification, and
...
Most recent broad verification recorded on 2026-05-19 during Pi executor
implementation:

- `bun run type-check`
- `bun run lint`
- `bun run test:unit:run` (`67 passed`)
- `python scripts/check_agent_repo.py`
- `uv run ruff check .`
- `uv run basedpyright` (`0 errors, 0 warnings, 0 notes`)
- `uv run pytest` (`160 passed, 1 skipped`)
...
- Full-stack smoke (one-time and recurring) was manually verified on the local
  stack in an earlier pass. Authenticated live Claude Code smoke was also
  manually verified previously.
- The optional authenticated Pi live smoke was not run in this session; setup
  notes live in `docs/local-full-stack-runbook.md`.
