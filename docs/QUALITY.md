...
| `apps/worker` | C | Workflow and executor structure exists; import hygiene, one-time replay, recurring materialization replay, and Claude Code runtime classification tests are covered. Authenticated live Claude Code and recurring smoke were manually verified on the local stack; they are not yet in automated CI coverage. |
